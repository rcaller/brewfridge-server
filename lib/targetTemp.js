...
function getTargetData(connection) {
  connection.query('SELECT timestampdiff(MINUTE,IFNULL(min(logtime), now()), now())/(60*24) AS t FROM measurements', [], function(err, results) {
    cTime = results[0].t;
...
function getCurrentTimeRelativeToStartPromise(connection) {
  var qqry = Q.ninvoke(connection, "query", 'SELECT IFNULL(timestampdiff(MINUTE, min(logtime), now())/(60*24), 0) AS t FROM measurements', []).then(function(data) {return data[0][0].t});
  return qqry;
}
